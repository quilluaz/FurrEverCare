...
export default function TreatmentPlans() {
  ...
  const fetchPlans = async (currentPetId) => { // Accept petId as argument
    // --- DEBUGGING LOGS ---
    console.log("fetchPlans called. UserID:", userID, "Selected PetID:", currentPetId);
    // --- END DEBUGGING LOGS ---
    ...
    try {
      console.log(`Fetching plans from: ${API_BASE_URL}/${userID}/pets/${currentPetId}/treatmentPlans`); // Use selectedPetId
      const res = await axios.get(`${API_BASE_URL}/${userID}/pets/${currentPetId}/treatmentPlans`);
      ...
      if (err.response?.status === 403) {
        ...
        window.location.href = "/login";
         ...
         setError(`Failed to fetch treatment plans. Status: ${err.response?.status || 'Network Error'}`);
      }
  ...
  const handleAddPlan = async (planData, accessToken) => {
    if (!userID || !selectedPetId) { // Use selectedPetId
    ...
    const tempPlan = { ...planData, planID: tempId, progressPercentage: planData.progressPercentage || 0 }; 
    ...
    try {
      ...
      const res = await axios.post(
        `${API_BASE_URL}/${userID}/pets/${selectedPetId}/treatmentPlans/add`,
      ...
      setPlans((p) =>
        p.map((pl) => (pl.planID === tempId ? { ...tempPlan, planID: realId } : pl))
      );
  ...
  const handleEditPlan = async (updatedPlanData, accessToken) => {
    if (!userID || !selectedPetId || !selectedPlan) { // Use selectedPetId
    ...
    setPlans((p) =>
      p.map((pl) =>
        pl.planID === planId ? { ...pl, ...updatedPlanData } : pl // Merge updates
      )
    );
  ...
  const handleDeletePlan = async () => {
    if (!userID || !selectedPetId || !planToDelete) { // Use selectedPetId
    ...
    try {
      await axios.delete(`${API_BASE_URL}/${userID}/pets/${selectedPetId}/treatmentPlans/${planId}`); // Use selectedPetId
      setLoadingPlanIds((ids) => ids.filter((id) => id !== planId));
  ...
  return (
    <div className="w-full min-h-screen bg-[#FFF7EC] font-['Baloo'] overflow-x-hidden">
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6 mt-8">
          ...
          <div className="flex items-center gap-3 flex-wrap"> {/* Added flex-wrap */}
            ...
            {userID && userPets.length > 0 && ( // Show dropdown only if logged in and pets are loaded
              <select
                id="pet-select"
                value={selectedPetId}
             ...
             {userID && userPets.length === 0 && !loading && !error && ( // Message if no pets and not loading/error
          ...
          <button
            ...
            disabled={!userID || !selectedPetId || loading || loadingPlanIds.length > 0} // Disable if not logged in, no pet selected, loading, or processing a plan
          >
        ...
        {error && (
          <p className="text-red-500 mb-4 text-center text-sm font-semibold">{error}</p>
        )}
        ...
        {loading ? ( // Show main loader if loading pets or plans initially
          <div className="flex flex-col items-center py-10">
            ...
            <p className="mt-2 text-gray-500 text-sm">
              {userPets.length === 0 ? 'Loading pets...' : 'Loading treatment plans...'}
            </p>
          </div>
        ) : !loading && plans.length === 0 && selectedPetId ? ( // Show 'no plans' only if not loading, plans are empty, and a pet IS selected
           <div className="flex flex-col items-center py-10">
             <p className="text-gray-500 text-sm">
               No treatment plans available for {userPets.find(p => p.petID === selectedPetId)?.name || 'this pet'}.
             </p>
           </div>
            ...
            <div className="flex flex-col items-center py-10">
             <p className="text-gray-500 text-sm">
               Please select a pet from the dropdown above to view their treatment plans.
             </p>
           </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {plans.map((plan) => (
              <div
                key={plan.planID}
                className={`bg-white rounded-lg shadow relative overflow-hidden text-sm transition-opacity duration-300 ${isPlanLoading(plan.planID) ? 'opacity-50' : 'opacity-100'}`}
              >
                ...
                <div className="p-4">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 break-words">{plan.description}</p>
                    </div>
                    <div className="flex gap-1 flex-shrink-0">
                      <button
                        onClick={() => confirmEditPlan(plan)}
                        disabled={!userID || isPlanLoading(plan.planID) || !selectedPetId} // Also disable if no pet selected
                  ...
                  {/* ... rest of plan details ... */}
                   <div className="grid grid-cols-2 gap-x-4 gap-y-2 mb-3">
                    <div>
                      <p className="text-[10px] font-medium text-[#042C3C]">Status</p>
                      <p className="text-xs text-gray-500">{plan.status || "N/A"}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-medium text-[#042C3C]">Progress</p>
                      <p className="text-xs text-gray-500">
                        {plan.progressPercentage !== undefined ? `${plan.progressPercentage}%` : "N/A"}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] font-medium text-[#042C3C]">Goal</p>
                      <p className="text-xs text-gray-500 break-words">{plan.goal || "None"}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-medium text-[#042C3C]">Start Date</p>
                      <p className="text-xs text-gray-500">
                         {formatDate(plan.startDate)}
                      </p>
                    </div>
                     <div>
                      <p className="text-[10px] font-medium text-[#042C3C]">End Date</p>
                      <p className="text-xs text-gray-500">
                         {formatDate(plan.endDate)}
                      </p>
                    </div>
                     <div className="col-span-2">
                      <p className="text-[10px] font-medium text-[#042C3C]">Notes</p>
                      <p className="text-xs text-gray-500 break-words">{plan.notes || "None"}</p>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <EditConfTreatmentPlanModal
        ...
        onConfirm={isEditConfirmation ? proceedWithEdit : () => setShowEditConfModal(false)} // proceedWithEdit only for confirmation prompt
